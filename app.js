$(document).ready( () => {
    const data = [
        {
            "img": `<svg width="79" height="79" xmlns="http://www.w3.org/2000/svg">
                    <path
    ...
    $('.period button').on('click', function () {
        displayContent($(this).data('timeframe'));
        $('.card p').animate({
            'transform': 'scale(1)'
        },500);
    });
    ...
    // Function to display content based on selected timeframe
    function displayContent(timeframe) {
        // Clear existing content
        $('.container .card').remove();

        // Iterate through data and create cards
        data.forEach(item => {
            const card = $(`
            <div class="card">
                <div class="card-img" style="background: ${item.color};">${item.img}</div>
                <div class="card-body">
                    <div class="card-flex">
                        <p class="card-title">${item.title}</p>
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-dots" width="25" height="25"
                    ...
                    <div class="card-flex flex-col">
                        <p class="card-time">${item.timeframes[timeframe].current}hrs</p>
                        <p class="card-info">Last week - ${item.timeframes[timeframe].previous}hrs</p>
                    </div>
                </div>
            </div>
        `);
